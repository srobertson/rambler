...
class Test(unittest.TestCase):
    ...
    def testWidgetThingyDoodle(self):
        query = "type=widget&query_select=name&query_select=thingy.name&query_select=thingy.doodle.name&query_select=doodle.name"
        sql = OQLQuery(query).query._sql
        print sql

    def testRelationQuery(self):
        query = "type=relation&sort_on=name&sort_order=ascending&fromKey=123&fromHome=widget&homeId=doodle&relation=widget_doodle&query_select=name&query_select=primaryKey"
        sql = OQLQuery(query).query._sql
        print sql
