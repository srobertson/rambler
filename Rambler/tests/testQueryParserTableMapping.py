...
class Test(unittest.TestCase):
    ...
    def testWidgetThingyDoodle(self):
        query = "type=widget&query_select=name&query_select=thingy.name&query_select=thingy.doodle.name&query_select=doodle.name&doodle.name=bleh"
        sql = OQLQuery(query).query._sql
        print sql
"""
    def testRelationQuery(self):
        query = "type=relation&sort_on=name&sort_order=ascending&fromKey=123&fromHome=widget&homeId=thingy&relation=widget_thingy&query_select=doodle.name&query_select=primaryKey&query_select=name"
        sql = OQLQuery(query).query._sql
